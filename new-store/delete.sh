...
echo "docker run --rm -it -w /current -v /var/run/docker.sock:/var/run/docker.sock -v $HOME/.ssh:/.ssh -v $(pwd):/current -v gcloud-data:/.config -v gcloud-data:/.kube -v gcloud-data:/.kubecfg -p 10443:443 -p 10080:80 --name kubectl yurifl/gcloud kubectl kubectl delete -f ./tmp"
